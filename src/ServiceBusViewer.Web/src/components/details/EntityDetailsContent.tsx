...
function getRuleKind(
  ...
  if (hasProperty(rule, 'sqlExpression')) {
    return 'sql';
  }

  if (
    hasProperty(rule, 'correlationId') ||
    hasProperty(rule, 'messageId') ||
    hasProperty(rule, 'applicationProperties')
  ) {
    return 'correlation';
  }

  if (hasProperty(rule, 'filterTypeName') || hasProperty(rule, 'filterExpression')) {
    return 'unknown';
  }
