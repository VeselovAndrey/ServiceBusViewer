...
/// <summary>
/// Represents an open Service Bus connection scoped to a single browser session. The connection exposes a
/// management-capable view of available entities and operations for peeking, receiving and sending messages against an
/// explicitly selected entity.
/// </summary>
public interface IServiceBusConnection : IAsyncDisposable
{
	...
	/// <summary>
	/// Indicates whether the Service Bus management API is available for this connection (used for entity discovery,
	/// management operations, etc.).
	/// </summary>
	bool IsManagementApiAvailable { get; }

	/// <summary>
	/// A read-only list of discovered entities in the connected namespace. Empty when discovery is unavailable or none
	/// found.
	/// </summary>
	IReadOnlyList<EntityProperties> AvailableEntities { get; }

	/// <summary>
	/// Refreshes the catalog of available entities from the management API.
	/// </summary>
	/// <returns>A task that completes when the refresh finishes.</returns>
	Task RefreshEntitiesAsync();

	/// <summary>
	/// Retrieves the properties for a specific entity previously discovered.
	/// </summary>
	...
	/// <summary>
	/// Peeks up to <paramref name="maxMessages"/> messages from the requested entity without removing them from the
	...
	/// <summary>
	/// Receives the next available message from the requested entity.
	/// </summary>
	/// <param name="entityId">Identifier of the entity to receive from.</param>
	/// <param name="sessionId">
	/// Session id to receive from, or <c>null</c> for non-session receive or next available session.
	/// </param>
	...
	/// <summary>
	/// Sends a message to the requested entity.
	/// </summary>
